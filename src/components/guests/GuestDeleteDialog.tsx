...
/**
 * Native <dialog> confirmation for deleting a guest that has constraints
 * referencing them (docs/10-interactions.md § 12). Uses showModal()/close()
 * so focus trapping and ESC handling come for free. Guests without
 * constraints are deleted directly with Undo available — no dialog.
...
export function GuestDeleteDialog({ guestName, constraintCount, onConfirm, onCancel }: GuestDeleteDialogProps) {
  ...
  return (
    ...
    >
      <h2 id="delete-guest-title" className="font-display text-lg font-semibold text-text">
