...
export function AddGuestSheet({ tableId, onClose }: AddGuestSheetProps) {
  ...
  if (empty.length === 0) {
    return (
      <Modal open onClose={onClose} title={format(t.placement.addToTable, { table: table.name })}>
  ...
  if (selectedGuest) {
    return (
      <Modal open onClose={onClose} title={format(t.placement.title, { name: selectedGuest.name })}>
  ...
  return (
    <Modal open onClose={onClose} title={format(t.placement.addToTable, { table: table.name })}>
      {candidates.length === 0 ? (
        <p className="text-sm text-text-muted">{t.placement.allSeated}</p>
      ) : (
          ...
          {candidates.map((guest) => (
            <li key={guest.id}>
              <button
                ...
                aria-label={format(t.placement.placeAtAriaLabel, { name: guest.name, table: table.name })}
